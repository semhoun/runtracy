...
class IdiormPanelTest extends BaseTestCase
{
    public function testIdiormPanel()
    {
        if (!class_exists('\ORM')) {
            ...
            $this->initIdiorm();
            $panel = new \SlimTracy\Helpers\IdiormPanel();
            $this->assertInstanceOf('\Tracy\IBarPanel', $panel);
            $this->assertInstanceOf('\SlimTracy\Helpers\IdiormPanel', $panel);
    ...
    {
        // empty data
        $result = $this->callProtectedMethod('parse', '\SlimTracy\Helpers\IdiormPanel', []);
        $this->assertRegexp('#No Logs#', $result);

        // some as in collector test
        $collector = new IdormCollector();
        $this->assertInstanceOf('\SlimTracy\Collectors\IdormCollector', $collector);
