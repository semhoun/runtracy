...
namespace Tests\SlimTracy\Helpers;
 ...
 * @package Tests\SlimTracy\Helpers
 */
class DoctrinePanelTest extends BaseTestCase
{
    public function testDoctrinePanel()
    {
        if (!class_exists('\Doctrine\DBAL\Configuration')) {
            ...
            $panel = new \SlimTracy\Helpers\DoctrinePanel(
                $logger
            );
    ...
    public function testDoctrinePanelParser()
    {
        ...
        $result = $this->callProtectedMethod('formatArrayData', '\SlimTracy\Helpers\DoctrinePanel', [
            $data[1]['params']
        ]);
        ...
        $result1 = $this->callProtectedMethod('formatArrayData', '\SlimTracy\Helpers\DoctrinePanel', [
            $data[1]['types']
        ]);
        $this->assertRegexp('#":id": "integer"#', $result1);
        // then transformNumericType
        $result2 = $this->callProtectedMethod('transformNumericType', '\SlimTracy\Helpers\DoctrinePanel', [
            $result1
        ]);
        // some
        ...
        // some twice
        $result3 = $this->callProtectedMethod('formatArrayData', '\SlimTracy\Helpers\DoctrinePanel', [
            $data[2]['types']
        ]);
        // one param
        ...
        // then transformNumericType
        $result4 = $this->callProtectedMethod('transformNumericType', '\SlimTracy\Helpers\DoctrinePanel', [
            $result3
        ]);
        // some
        ...
        // parse($data)
        $result = $this->callProtectedMethod('parse', '\SlimTracy\Helpers\DoctrinePanel', [$data]);
